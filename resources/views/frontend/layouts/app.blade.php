...
<head>
    ...
    <link rel="stylesheet" href="{{ static_asset('assets/css/vendors.css?v=1') }}">
    <link rel="stylesheet" href="{{ static_asset('assets/css/lineawesome.css') }}">
    <link rel="stylesheet" href="{{ static_asset('assets/css/bootstrap-select.css') }}">
    <link rel="stylesheet" href="{{ static_asset('assets/css/slickcarousel.css') }}">
    @yield('style')
    <link rel="stylesheet" href="{{ static_asset('assets/css/aiz-core.css?v=1.1') }}">
    <link rel="stylesheet" href="{{ static_asset('assets/css/custom-style.css?v=1.0') }}">
    <script>
        var AIZ = AIZ || {};
    </script>
    ...
    @endphp
    <script>
        var ajax_loader_small =
            '<div class="spinner-border  text-dark" role="status" style="border-right-color: black !important;"><span class="sr-only">Loading...</span></div>';
    </script>
</head>
...
<body>
    ...
    <script src="{{ static_asset('assets/js/bootstrap-select.js') }}"></script>
    <script src="{{ static_asset('assets/js/lazaysizes.js') }}"></script>
    <script src="{{ static_asset('assets/js/notify.js') }}"></script>
    <script src="{{ static_asset('assets/js/jquery.meanmenu.js') }}"></script>
    ...
    <script>
        ...
        function getPackagePrice() {
            $('#product_total').html(ajax_loader_small);
            $.ajax({
                type: "POST",
                url: '{{ route('package.total') }}',
                data: $('#option-choice-form').serializeArray(),
                success: function(data) {
        ...
        function checkAddToCartValidityPackage() {
            var names = {};
            $('#option-choice-form select').each(function() { // find unique names
                names[$(this).attr('name')] = true;
            });
            var count = 0;
            var select_count = 0;
        ...
        function buyNow(elem) {
            ...
            if (checkAddToCartValidityPackage()) {
                elem.prop('disabled', true);
                $('.actBtn-loader').show();
                $.ajax({
                    type: "POST",
                    url: '{{ route('cart.addToCartPackage') }}',
                    data: $('#option-choice-form').serializeArray(),
                    success: function(data) {
                        if (data.cart_step == 1) {
                            window.location.href = "{{ route('confirm_order') }}";
                        } else if (data.cart_step == 0) {
                            window.location.href = "{{ route('start_project') }}";
                        } else {
