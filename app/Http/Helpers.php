...
//highlights the selected navigation on admin panel
if (!function_exists('areCloseRoutes')) {
    function areCloseRoutes(array $routes, $output = "side-menu-closed")
    {
        foreach ($routes as $route) {
            if (Route::currentRouteName() == $route) return $output;
        }
...
if (! function_exists('isUniqueEmailUpdate')) {
    /**
     ...
     * @return string
     */
    function isUniqueEmailUpdate($email,$id)
    {
        $user = \App\User::where('email', $email)->where('id','!=',$id)->first();
        if($user == null) {
